...
from models.ablation_model import GRU_CNN_Attention as HMULSES_DST
...
def train_epoch(model, dataloader, optimizer, device, args):
    ...
    for batch in tqdm(dataloader, desc="训练中"):
        ...
        # 修复：结合 use_ordered_loss 开关和 loss_type 参数
        loss = loss_function(
            output, label,
            loss_type="ordered" if args.use_ordered_loss else args.loss_type,
            expt_type=args.class_num,
            scale=1.8
        )
        loss.backward()
...
def parse_args():
    ...
    # 数据与路径
    parser.add_argument("--data_path", type=str, default='data/reddit_clean.pkl', help='数据路径')
    ...
    parser.add_argument("--use_gated_fusion", action="store_true", default=True,
                      help="是否使用门控融合")
    parser.add_argument("--use_ordered_loss", action="store_true", default=True,
                      help="是否使用有序损失")
    parser.add_argument("--word_level_only", action="store_true", default=False,
                      help="是否仅使用词级特征")
    # 添加 loss_type 参数，用于指定损失函数类型
    parser.add_argument("--loss_type", type=str, default="ordered", choices=["ordered", "cross_entropy"],
                      help="损失函数类型（ordered=有序损失，cross_entropy=交叉熵）")
...
def main():
    ...
    # 在main函数中
    print(f"消融配置:")
    print(f"  使用全局注意力: {args.use_global_attention}")
    print(f"  使用门控融合: {args.use_gated_fusion}")
    print(f"  使用有序损失: {args.use_ordered_loss}")
    print(f"  仅词级特征: {args.word_level_only}")
